...
import { ShoppingCart, FileText, CheckCircle, Package, Download, Plus, Filter, ArrowRight } from 'lucide-react';
...
export default function SalesOrdersPage() {
  ...
  async function handleCreateOrder() {
    ...
    const totalAmount = selectedProd ? selectedProd.price * qty : 0;

    await inventoryApi.createSalesOrder({
      customerId: form.customerId,
      items: [{ productId: form.productId, quantity: qty, unitPrice: selectedProd?.price || 0 }],
      totalAmount,
    });
    setShowModal(false);
    ...
    <div className="fade-in">
      ...
      <div className="kpi-grid" style={{ marginBottom: '24px' }}>
        ...
        <div className="kpi-card">
          ...
          <div className="kpi-card-value">${orders.reduce((acc, curr) => acc + (curr.totalAmount || 0), 0).toLocaleString()}</div>
        ...
        <table className="data-table">
          ...
          <tbody>
            {filteredOrders.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: '20px' }}>No sales orders found</td></tr>
            ) : filteredOrders.map((o, i) => (
              <tr key={o.id || i}>
                <td style={{ fontWeight: 600, color: 'var(--color-primary)' }}>{o.orderNumber || `SO-${i+1001}`}</td>
      ...
      <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Create Sales Order">
        ...
        <FormField label="Product Item" type="select" value={form.productId} onChange={(v) => setForm({ ...form, productId: v })} required
